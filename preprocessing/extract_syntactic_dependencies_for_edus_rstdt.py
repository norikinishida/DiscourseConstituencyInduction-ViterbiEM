...
def main():
    ...
    for filename in filenames:
        # Path
        path_edus = os.path.join(config.getpath("data"), "rstdt", "renamed", filename + ".preprocessed")
        path_postags = os.path.join(config.getpath("data"), "rstdt", "renamed", filename + ".postags")
        path_conll = os.path.join(config.getpath("data"), "rstdt", "tmp.preprocessing", filename.replace(".edus", ".sentences.conll"))
        path_sbnds = os.path.join(config.getpath("data"), "rstdt", "renamed", filename.replace(".edus", ".sentence.noproj.boundaries"))
        path_out1 = os.path.join(config.getpath("data"), "rstdt", "renamed", filename + ".arcs") # e.g., "train.123.edus.arcs" (!= "train.123.arcs")
        path_out2 = os.path.join(config.getpath("data"), "rstdt", "renamed", filename + ".heads")
