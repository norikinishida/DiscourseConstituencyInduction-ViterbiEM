...
def main():
    ...
    for filename in filenames:
        # Path
        path_edus = os.path.join(config.getpath("data"), "rstdt", "tmp.preprocessing", filename + ".tokenized")
        path_conll = os.path.join(config.getpath("data"), "rstdt", "tmp.preprocessing", filename.replace(".edus", ".sentences.conll"))
        path_out = os.path.join(config.getpath("data"), "rstdt", "renamed", filename + ".postags")
