...
def evaluate(pred_path, gold_path):
    ...
    :rtype: {str: {str: Any}}
    """
    pred_trees = read_trees(pred_path)
    gold_trees = read_trees(gold_path)
    ...
    pred_spans = [get_spans(tree) for tree in pred_trees]
    gold_spans = [get_spans(tree) for tree in gold_trees]
    scores = parseval(pred_spans, gold_spans, input_lengths)
    return scores
...
def parseval(preds, golds, input_lengths):
    """
